...
def main():
    st.set_page_config(page_title="Titanic - Przewidywanie Przeżycia")
    overview = st.container()
    left, right = st.columns(2)
    prediction = st.container()

    st.image("https://snipboard.io/GCcVZD.jpg")

    with overview:
        st.title("Titanic - Przewidywanie Przeżycia")

    with left:
        pclass_radio = st.radio("Klasa pasażera", list(pclass_d.keys()), format_func=lambda x: pclass_d[x])
